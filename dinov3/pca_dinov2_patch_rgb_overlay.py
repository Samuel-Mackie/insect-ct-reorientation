...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Patch-wise DINOv3 PCA (RGB) with foreground filtering for one animal."
    )
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path("data/new_photos_dinov3/pca_patch_rgb"),
    ...
    parser.add_argument(
        ...
        type=int,
        default=16,
        help="Patch size in pixels for output visualization; use 16 to match DINOv3 patch tokens.",
    )
    parser.add_argument(
        "--device",
        type=str,
...
def main() -> None:
    ...
    model_patch = int(getattr(model.config, "patch_size", 16))
