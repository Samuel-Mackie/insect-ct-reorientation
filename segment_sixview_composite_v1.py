    ...
    try:
        with tempfile.TemporaryDirectory() as td:
            ...
            for idx, label, direction, view_up in VIEWS:
                ...
                shot = tdir / f"tmp_{idx}_{label}.png"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--input-root",
        type=Path,
        default=Path("data/finished_photos/v1/rotated"),
        help="Input root, typically data/finished_photos with <species>/tif/*.tif.",
    )
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path("data/finished_photos/v1/composite"),
...
def main() -> None:
    ...
    for i, in_path in enumerate(volumes, start=1):
        ...
        try:
            ...
            mask = segment_largest_component(data, threshold_percentile=cfg.threshold_percentile)
